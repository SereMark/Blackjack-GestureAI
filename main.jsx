...
const CONFIG = {
  ...
  dealerStandsOn: 17,
  gestureHoldTime: 800      // ms required to confirm a hand-gesture
}
...
const useGameStore = create((set, get) => ({
  ...
  deal: () => {
    ...
    set({ deck, playerHand: [], dealerHand: [], balance: balance - bet,
          gamePhase: 'dealing', message: 'Dealing...', showDealerCards: false })
  ...
  checkForBlackjack: (p, d) => {
    ...
    if (pv === 21 || dv === 21) {
      ...
      if (pv === 21 && dv === 21)
        ...
        get().endGame(`Blackjack! You win $${get().bet * CONFIG.blackjackPayout}!`,
  ...
  stand: () => {
    if (get().gamePhase !== 'playing') return
    set({ gamePhase: 'dealer', showDealerCards: true, message: "Dealer's turn..." })
    setTimeout(() => get().dealerPlay(), 1200)
  },

  dealerPlay: () => {
    const { deck, dealerHand } = get()
    if (calculateValue(dealerHand) >= CONFIG.dealerStandsOn) return get().endGame()
  ...
  endGame: (msg, winnings = null) => {
    ...
    if (winnings === null) {                              // normal settlement
      const pv = calculateValue(playerHand), dv = calculateValue(dealerHand)
    ...
    set({ balance: balance + (winnings ?? 0), gamePhase: 'ended',
          message: msg, showDealerCards: true })
  },

  resetRound: () => {
    const { balance } = get()
    if (balance < CONFIG.minBet)
      return set({ gamePhase: 'ended',
                   message: 'Game Over! Not enough money to play.' })
    set({ deck: [], playerHand: [], dealerHand: [],
          gamePhase: 'betting', message: 'Place your bet for the next round.',
  ...
  startNewGame: () => set({
    ...
    deck: [], playerHand: [], dealerHand: [],
    gamePhase: 'betting', message: 'Place your bet to start!',
    showDealerCards: false
  })
}))
...
const Card = ({ rank, suit, hidden = false }) => {
  ...
  return (
    <motion.div layout initial={{ scale:.5, y:-100, opacity:0 }}
                animate={{ scale:1, y:0, opacity:1 }}
                exit={{ scale:.5, y:100, opacity:0 }}
      ...
      <div className="relative w-full h-full transition-transform duration-700
        ...
        <div className={`absolute w-full h-full [backface-visibility:hidden]
                         rounded-lg bg-white shadow-xl border-2 ${color}
...
const useGestureDetection = () => {
  ...
  const { hit, stand, gamePhase } = useGameStore()

  useEffect(() => {                           // initialise / teardown
    if (!enabled) {
    ...
    ;(async () => {
      ...
      } catch (e) { console.error('Gesture init failed', e); mounted&&setEnabled(false) }
    })()
    return () => { mounted = false }
  }, [enabled])

  useEffect(() => {                           // detection loop
    if (!ready || !recognizerRef.current || gamePhase!=='playing') {
      setGesture(null); return
    }
    let last = null, start = 0, id
    const detect = ts => {
      const res = recognizerRef.current.recognizeForVideo(videoRef.current, ts)
      const g   = res.gestures?.[0]?.[0]
      if (g && g.score > .7) {
        if (g.categoryName !== last) { last = g.categoryName; start = Date.now() }
        const prog = Math.min((Date.now()-start) / CONFIG.gestureHoldTime, 1)
        setGesture({ name:g.categoryName, progress:prog })
        if (prog === 1) { last==='Open_Palm' ? hit() : stand(); last = null }
      } else { setGesture(null); last = null }
...
function App() {
  ...
  const view = { betting:S.gamePhase==='betting',
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4 font-sans flex flex-col">
      <header className="text-center mb-4">
        <h1 className="text-4xl md:text-5xl font-bold tracking-wider">Blackjack</h1>
        <p className="text-xl md:text-2xl mt-1">
          Balance: <span className="font-semibold text-green-400">${S.balance}</span>
        </p>
      </header>

      <main className="flex-grow grid md:grid-cols-3 gap-6 max-w-7xl mx-auto w-full">
        {/* === table === */}
        <div className="md:col-span-2 bg-green-800 bg-[radial-gradient(theme(colors.green.900)_1.5px,transparent_1.5px)] [background-size:20px_20px] rounded-xl p-4 md:p-6 shadow-2xl flex flex-col">
          {/* dealer */}
          <div className="flex-1">
            <h2 className="text-lg md:text-xl mb-3 text-center opacity-80">
              Dealer's Hand {S.dealerHand.length>0 &&
                `(${S.showDealerCards ? dVal : calculateValue([S.dealerHand[1]])})`}
            </h2>
            <div className="flex flex-wrap gap-3 justify-center min-h-[180px] items-center">
              <AnimatePresence>
                {S.dealerHand.map((c,i)=>(
                  <Card key={c.id} {...c} hidden={!S.showDealerCards && i===0}/>
                ))}
              </AnimatePresence>
            </div>
          </div>
          ...
          {/* status */}
          <div className="text-center text-xl md:text-2xl font-bold my-4 min-h-[60px] flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.div key={S.message}
          ...
          <div className="flex-1">
            <h2 className="text-lg md:text-xl mb-3 text-center opacity-80">
              Your Hand {S.playerHand.length>0 && `(${pVal})`}
            </h2>
            <div className="flex flex-wrap gap-3 justify-center min-h-[180px] items-center">
              <AnimatePresence>
          ...
          {/* controls */}
          <div className="flex justify-center items-center gap-4 flex-wrap mt-4 min-h-[52px]">
            {view.betting && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}}
                          className="flex items-center gap-4">
                <label className="flex items-center gap-2 font-semibold">
                  Bet:
                  <input type="number" value={S.bet}
                         onChange={e=>S.setBet(+e.target.value)}
                         min={CONFIG.minBet} max={S.balance} step="10"
                         className="px-3 py-2 rounded bg-gray-700 w-28 border border-gray-600
                                    focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                </label>
                <button onClick={S.deal}
                        disabled={S.bet>S.balance || S.bet<CONFIG.minBet}
                        className="px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-bold disabled:opacity-50 shadow-lg">
                  Deal
                </button>
              </motion.div>
            )}
            ...
            {view.playing && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}}
                          className="flex items-center gap-4">
                <button onClick={S.hit}
                        className="px-6 py-3 bg-green-600 hover:bg-green-700 rounded-lg font-bold shadow-lg">
                  Hit (H)
                </button>
                <button onClick={S.stand}
                        className="px-6 py-3 bg-red-600 hover:bg-red-700 rounded-lg font-bold shadow-lg">
                  Stand (S)
                </button>
              </motion.div>
            )}
            ...
            {view.ended && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}}>
                <button onClick={gameOver ? S.startNewGame : S.resetRound}
                        className="px-6 py-3 bg-yellow-500 hover:bg-yellow-600 text-black rounded-lg font-bold shadow-lg">
        ...
        <div className="bg-gray-800 rounded-xl overflow-hidden flex flex-col">
                  ...
                  <div className="absolute bottom-0 inset-x-0 bg-black/70 p-3 backdrop-blur-sm">
                    ...
                    {view.playing && (
                      <div className="w-full bg-gray-600 rounded-full h-2.5 overflow-hidden">
                        <motion.div className="bg-blue-500 h-full"
          ...
          <div className="p-4 text-sm text-center text-gray-400">
            <p>✋ <span className="font-semibold">Open Palm</span> = Hit</p>
            <p>✊ <span className="font-semibold">Closed Fist</span> = Stand</p>
            <p className="mt-2 text-xs">Hold gesture to confirm or press H / S.</p>
          </div>
        </div>
      </main>
    </div>
