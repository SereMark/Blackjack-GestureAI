...
class MediaPipeRecognizer {
  ...
  async close() { await this.recog?.close(); this.recog = null; }
}
  ...
  (['♠','♥','♦','♣'] as Suit[])
    .flatMap(s =>
      (['A','K','Q','J','10','9','8','7','6','5','4','3','2'] as Rank[])
        .map(r => ({ suit:s, rank:r, id:`${r}-${s}-${Math.random()}` }))
    )
    .sort(() => Math.random() - 0.5);

/** guard undefined placeholders (face-down dealer card) */
const calc = (hand: (Card | undefined)[]) => {
  let v=0, aces=0;
  hand.forEach(c=>{
    if(!c) return;
...
const useGame = create<Store>((set, get) => ({
  ...
  /* log */
  logs: [], pushLog: l => set(st => ({ logs:[l, ...st.logs.slice(0,19)] })),
  ...
  deal: () => {
    ...
    later(() => set({ playerHand:[p[0]] }), 200);
    later(() => set({ dealerHand:[d[0]] }), 600);
    later(() => set({ playerHand:p      }),1000);
    later(() => set({ dealerHand:d      }),1400);
    later(() => get().checkBJ(p,d)      ,2000);
  },
  ...
  double: () => {
    ...
    const deck=[...get().deck], hand=[...playerHand, deck.pop()!];
  ...
  next: () => {
    ...
    set({
      deck:[], playerHand:[], dealerHand:[],
      phase:'betting', msg:'Place your bet.',
...
const Ring:React.FC<{progress:number}> = ({progress})=>(
  <svg viewBox="0 0 120 120" className="w-24 h-24">
    <circle cx="60" cy="60" r="54" stroke="#4b5563" strokeWidth="12" fill="none"/>
    <motion.circle cx="60" cy="60" r="54" stroke="#3b82f6" strokeWidth="12"
      fill="none" strokeLinecap="round"
      animate={{ pathLength:progress }} transition={{ duration:.1 }}/>
  </svg>
);
...
const Calibrate:React.FC<{onClose:()=>void}> = ({ onClose }) => {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const r=new MediaPipeRecognizer(); await r.init(videoRef.current!);
  ...
  useEffect(()=>{
    ...
    const loop=(ts:number)=>{
      ...
      if(g && g.categoryName===settings[steps[idx].key] && g.score>=0.9){
        ...
        setIdx(i=>i+1); return rafRef.current=requestAnimationFrame(loop);
      }
      ...
      if(g && g.score>=0.75){
        ...
        if(p===1){
          ...
          setDet(null); setProg(0);
        }
      }else{ setDet(null); setProg(0); }
    ...
    <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50">
      <div className="bg-gray-800 w-full max-w-md rounded-xl shadow-xl overflow-hidden">
        ...
        <div className="aspect-video bg-black relative flex items-center justify-center">
          ...
          {!ready && <div className="absolute inset-0 flex items-center justify-center text-gray-400">Starting camera…</div>}
          {det   && <div className="absolute inset-0 flex items-center justify-center"><Ring progress={prog}/></div>}
        </div>

        <div className="p-4 text-center space-y-3">
          <p>Hold the <span className="font-semibold">{current}</span> gesture until the ring completes.</p>
          <p className="text-xs text-gray-400">Ensure good lighting and keep your hand centred.</p>
        </div>
      </div>
    </div>
  );
...
/* ── 7 · card component ───────────────────────────────────────────────────── */
const Card:React.FC<{rank:Rank; suit:Suit; hidden?:boolean}> = ({
  rank,suit,hidden
})=>{
  ...
  return(
    <motion.div layout initial={{scale:.5,y:-80,opacity:0}}
      ...
      <div className="relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d]"
        ...
        <div className={`absolute inset-0 [backface-visibility:hidden] bg-white rounded-lg border-2 shadow-xl flex flex-col justify-between p-2 font-bold ${color}`}>
          <div>{rank} {suit}</div>
          <div className="text-5xl md:text-6xl text-center">{suit}</div>
          <div className="rotate-180 self-end">{rank} {suit}</div>
        </div>
        <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-lg bg-gradient-to-br from-blue-600 to-blue-800 border-2 border-blue-900"/>
      </div>
    </motion.div>
  );
};
...
const play = (k: keyof typeof sounds, muted:boolean) =>
  !muted && sounds[k].play().catch(()=>{/* ignore */});
...
const useBrightness = (videoRef:RefObject<HTMLVideoElement>) => {
  useEffect(()=>{
    ...
    const check=()=>{
      ...
      if(avg<40||avg>220) toast('Lighting may be poor ⚠',{id:'light',icon:'💡'});
      else toast.dismiss('light');
      schedule();
    };
    const schedule=()=>{ timer=window.setTimeout(check,4000); };
    schedule();
...
const useGesture = () => {
  ...
  const G = useGame();
  const {
    hitGesture, standGesture, doubleGesture,
    holdTime, confidence, autoLearn, muted, leftHand
  } = G.settings;

  /* suspend on hidden tab */
  useEffect(()=>{
    const vis=()=>{ if(document.hidden) setEnabled(false); };
  ...
  useEffect(()=>{
    if(!enabled){
      ...
      videoRef.current?.srcObject &&
        (videoRef.current.srcObject as MediaStream).getTracks().forEach(t=>t.stop());
      setReady(false); setGesture(null); return;
    }

    let live=true;
    (async()=>{
      try{
        const recog=recognizerFactory(); await recog.init(videoRef.current!);
        ...
        videoRef.current!.srcObject=stream;
        videoRef.current!.onloadedmetadata=()=>setReady(true);
      }catch(e){ toast.error('Camera / AI initialisation failed'); console.error(e); setEnabled(false); }
    })();
    return()=>{ live=false; };
  },[enabled]);
  ...
  useEffect(()=>{
    ...
    let last:string|null=null, start=0, quiet=0;
    const loop=(ts:number)=>{
      ...
      if(g && g.score>=confidence){
        ...
        if(cat!==last){ last=cat; start=Date.now(); quiet=0; }
        ...
        setGesture(null);
        if(++quiet===180){ toast('No hand detected',{id:'nohand',icon:'🖐️'}); quiet=0; }
      }
      rafRef.current=requestAnimationFrame(loop);
    };
    rafRef.current=requestAnimationFrame(loop);
    ...
    ready,confidence,holdTime,
    hitGesture,standGesture,doubleGesture,
    leftHand,muted,autoLearn,G
  ]);
...
const App:React.FC = () => {
  ...
  useEffect(()=>{ addEventListener('keydown',onKey); return()=>removeEventListener('keydown',onKey); },[onKey]);

  useEffect(()=>{ if(cal) setEnabled(false); },[cal,setEnabled]);
  ...
  return(
    <div className="min-h-screen bg-gray-900 text-white p-4 flex flex-col">
      ...
      <header className="text-center mb-4">
        ...
        <p className="text-xl">
          Balance:&nbsp;<span className="font-semibold text-green-400">${G.balance}</span>
        </p>
      </header>
      ...
      <main className="flex-grow grid md:grid-cols-3 gap-6 max-w-7xl mx-auto w-full">
        ...
        <div className="md:col-span-2 bg-green-800 bg-[radial-gradient(#064e3b_1.5px,transparent_1.5px)] [background-size:20px_20px] rounded-xl p-4 shadow-2xl flex flex-col">
          ...
          <div className="flex justify-center gap-4 flex-wrap mt-4 min-h-[52px]">
            ...
            {G.phase==='playing' && <>
              <button onClick={G.hit}
                      className="px-6 py-2 bg-green-600 rounded-lg font-bold">Hit (H)</button>
              <button onClick={G.stand}
                      className="px-6 py-2 bg-red-600 rounded-lg font-bold">Stand (S)</button>
              <button onClick={G.double}
          ...
          <div className="p-4 bg-gray-900 flex justify-between items-center">
            ...
            <div className="flex gap-2 text-xs items-center">
              ...
              <button onClick={()=>setCal(true)}
                      className="bg-gray-700 px-2 py-1 rounded">Calibrate</button>
          ...
          {drawer &&
            <div className="p-4 bg-gray-900 border-t border-gray-700 text-sm space-y-3 overflow-y-auto">
              {(['hitGesture','standGesture','doubleGesture'] as const).map(k=>{
                const usedBy = (g:string)=> (['hitGesture','standGesture','doubleGesture'] as const)
                  .some(other => other!==k && G.settings[other]===g);
                return (
